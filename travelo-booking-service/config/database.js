    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      try {
        ...
        if (attempt === MAX_ATTEMPTS) {
          console.error("***ERROR*** BOOKING SERVICE database connection", err.message);
          throw err;
        }
        const waitMs = 2000 * 2 ** (attempt - 1);
        console.error(
          `***ERROR*** BOOKING SERVICE database connection (pokušaj ${attempt}/${MAX_ATTEMPTS}): ${err.message} — ponovni pokušaj za ${waitMs / 1000}s`
        );
