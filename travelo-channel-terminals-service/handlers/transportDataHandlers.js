...
//
// Kad backoffice ne odgovori, greška se propušta dalje i plovidbeni red se NE
...
// i nazivi plovidbenog reda koje nigdje ne prikazuje. Uz 4000 polazaka to je razlika
...
// Kratka memorija posljednjeg slozenog plovidbenog reda, po uredaju. Terminal
// ga osvjezava cesto — pri prijavi, na zahtjev, pri promjeni dana — a svaki put
// se iznova dohvacaju luke, linije, polasci i cjenici iz sales-servisa i
// filtriraju po zabranjenim linijama. Plovidbeni red se ne mijenja u sekundama,
// pa se minutu drzi slozen paket.
//
// Kad se polazak otkaze ili pomakne, memorija se brise odmah (vidi
// syncSignalWatcher): inace bi uredaj, koji na javljanje reagira u nekoliko
// sekundi, dobio upravo onu sliku od prije promjene i izgledalo bi kao da
// javljanje ne radi.
...
const transportDataHandlers = async(billingDeviceUuid, opcije = {})=>{
    try {
        ...
        // 200 bez podataka i nije znao da plovidbeni red nije osvježen.
        console.log('[transport_data] neuspjeh:', error?.message || error)
        throw error
    }
}

// Brisanje memorije kad se plovidbeni red promijeni.
const ocistiMemorijuPlovidbenogReda = () => {
    memorija.clear();
};
