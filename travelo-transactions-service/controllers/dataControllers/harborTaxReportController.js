...
// Izračun je izdvojen iz kontrolera jer ga uz JSON izvještaj koristi i PDF za
// preuzimanje (harborTaxPdfController) — tako obje verzije pokazuju iste brojke.
const buildHarborTaxReport = async ({ TicketsModel, year, month }) => {
    let start, end;
    if (month) {
    ...
    const period = { year, month: month || null };
    if (rows.length === 0) {
        return { period, total_harbor_tax: 0, total_tickets: 0, by_region: [] };
    }
    ...
    return {
        period,
        total_harbor_tax: +by_region.reduce((s, r) => s + r.total, 0).toFixed(2),
        total_tickets: by_region.reduce((s, r) => s + r.tickets, 0),
        by_region,
    };
};
...
module.exports = { harborTaxReportController, buildHarborTaxReport, norm, HARBOR_RATE };
