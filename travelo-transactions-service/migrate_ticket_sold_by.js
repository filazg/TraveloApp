// Dodaje stupac s korisnickim imenom onoga tko je kartu prodao kroz partnersku
// prodaju. Partner ima vise korisnika, a obracun provizije razraduje promet po
// osobi.
//
...
//
// Pokretanje:  node migrate_ticket_sold_by.js
//              APP_ENV=test_do DB_PASS='...' node migrate_ticket_sold_by.js
//
...
(async () => {
    ...
    try {
        ...
        await sequelize.query(`
            ALTER TABLE tickets
                ADD COLUMN IF NOT EXISTS sold_by_username VARCHAR(255);
        `);
        const [stupci] = await sequelize.query(`
            SELECT column_name FROM information_schema.columns
            WHERE table_name = 'tickets' AND column_name = 'sold_by_username'
        `);
        console.log("tickets — stupac:", stupci.map((c) => c.column_name).join(", ") || "NEMA");
    } catch (err) {
