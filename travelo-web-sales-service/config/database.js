  ...
  if (!sequelize) {
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      try {
        ...
        console.log("WEB SALES SERVICE database connected...");
        break;
      } catch (err) {
        if (attempt === MAX_ATTEMPTS) {
          console.error("***ERROR*** WEB SALES SERVICE database connection", err.message);
          throw err;
        }
        const waitMs = 2000 * 2 ** (attempt - 1);
        console.error(
          `***ERROR*** WEB SALES SERVICE database connection (pokušaj ${attempt}/${MAX_ATTEMPTS}): ${err.message} — ponovni pokušaj za ${waitMs / 1000}s`
        );
