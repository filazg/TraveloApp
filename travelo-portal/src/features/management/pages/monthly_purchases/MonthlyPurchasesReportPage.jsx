...
export default function MonthlyPurchasesReportPage() {
    ...
    const toggleLine = (code) => {
        setExpandedLines((s) => { const n = new Set(s); n.has(code) ? n.delete(code) : n.add(code); return n; });
    };
    const togglePolazak = (key) => {
        setExpandedPolasci((s) => { const n = new Set(s); n.has(key) ? n.delete(key) : n.add(key); return n; });
    };
    const toggleLeg = (key) => {
        setExpandedLegs((s) => { const n = new Set(s); n.has(key) ? n.delete(key) : n.add(key); return n; });
    };

    const days = f.purchaseReport?.days || 0;
    const lines = f.purchaseReport?.lines || [];
    ...
    return (
        <Box sx={{ width: "100%", p: 2 }}>
            <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 2 }}>
                ...
                <Typography variant="h5" fontWeight={700}>Prodaja</Typography>
                <Typography variant="body2" color="text.secondary" sx={{ ml: 1 }}>(po datumu kupnje)</Typography>
            </Stack>
            ...
            <Paper variant="outlined" sx={{ p: 2, mb: 2, borderRadius: 2 }}>
                <Stack direction="row" spacing={2} alignItems="center">
                    <TextField
                        ...
                        value={month}
                        onChange={(e) => dispatch(setPurchaseReportMonth(e.target.value))}
                        sx={{ width: 200 }}
                    />
                    <Button
                        variant="contained"
                        startIcon={<SearchIcon />}
                        onClick={() => dispatch(fetchPurchaseReportThunk(month))}
                    >Osvježi</Button>
                    <ToggleButtonGroup
                        size="small"
                        value={viewMode}
                    ...
                    {f.purchaseReport && (
                        <Typography variant="body2" color="text.secondary">
                            Karata u obračunu: <b>{f.purchaseReport.tickets_matched ?? 0}</b>
                            &nbsp;·&nbsp; {viewMode === "amount"
            ...
            {f.purchaseReportError && <Alert severity="error" sx={{ mb: 2 }}>{f.purchaseReportError}</Alert>}
            {!f.purchaseReportLoading && lines.length === 0 && (
