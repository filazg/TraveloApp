import { useCallback } from "react";
import { useDispatch } from "react-redux";
import { setAuthData } from "../auth/authSlice";

// Globalni prekrivač s porukom — LoadingOverlay ga crta iz `auth.loading`.
// Svaka radnja koja traje dulje od trenutka mora ga podići: inače korisnik ne
// zna radi li se išta, pa klikne drugi put.
//
// Funkcije su memoizirane (useCallback) da im referenca bude stabilna: inače
// komponente koje ih stave u useEffect/useCallback ovisnosti ulaze u beskonačnu
// petlju (npr. DownloadsPage je time neprestano dohvaćao /downloads/list).
export function useLoading() {
    const dispatch = useDispatch();

    const pokazi = useCallback((poruka) => {
        dispatch(setAuthData({ path: "loadingMessage", value: poruka || "Molimo pričekajte" }));
        dispatch(setAuthData({ path: "loading", value: true }));
    }, [dispatch]);

    const sakrij = useCallback(() => dispatch(setAuthData({ path: "loading", value: false })), [dispatch]);

    // Pokriva i slučaj kad radnja pukne — bez `finally` prekrivač ostane visjeti
    // preko ekrana i aplikacija izgleda zamrznuto.
    const tijekom = useCallback(async (poruka, radnja) => {
        pokazi(poruka);
        try {
            return await radnja();
        } finally {
            sakrij();
        }
    }, [pokazi, sakrij]);

    return { pokazi, sakrij, tijekom };
}
